...
public class AsymmetryBasedInconsistencyFinder extends AbstractTrivialInconsistencyFinder {
	...
	private ParameterizedSparqlString template = new ParameterizedSparqlString("SELECT * WHERE {?s ?p ?o. ?o ?p ?s.}");
	...
	private Set<OWLObjectProperty> generatePropertyCandidates(){
		...
		String query = "SELECT ?p WHERE {?p a <http://www.w3.org/2002/07/owl#AsymmetricProperty>.}";
		...
		for (Iterator<OWLObjectProperty> iterator = propertyCandidates.iterator(); iterator.hasNext();) {
			if(!terminationCriteriaSatisfied()){
				...
				while (rs.hasNext()) {
					...
					ax1 = dataFactory.getOWLObjectPropertyAssertionAxiom(property, subject, object);
					ax2 = dataFactory.getOWLObjectPropertyAssertionAxiom(property, object, subject);
					justification = Sets.newHashSet(ax1, ax2, dataFactory.getOWLAsymmetricObjectPropertyAxiom(property));
