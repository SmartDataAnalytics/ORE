...
public class OptimizedDisjointnessBasedInconsistencyFinder extends AbstractTrivialInconsistencyFinder {
	...
	private static final Logger logger = Logger
			.getLogger(OptimizedDisjointnessBasedInconsistencyFinder.class.getName());
	...
	public OptimizedDisjointnessBasedInconsistencyFinder(SparqlEndpointKS ks) {
		super(ks);
	...
	public Set<OWLAxiom> getInconsistentFragment() throws TimeOutException {
		Set<OWLAxiom> fragment = new HashSet<>();
		...
		for (OWLDisjointClassesAxiom ax : disjointnessAxioms) {
			logger.info("Analyzing candidate: " + ax);
			
			//check A(x),B(x)
			if(currentState == State.CLASS_ASSERTION){
				fragment.addAll(analyzeClassAssertionViolation(ax));
				currentState = State.DOMAIN_VIOLATION;
				if(stopIfInconsistencyFound && !fragment.isEmpty()){
					break;
				}
			}
			
			//check A(x), Dom(p, B), p(x,y)
			if(currentState == State.DOMAIN_VIOLATION){
				fragment.addAll(analyzeDomainViolation(ax));
			...
			if(currentState == State.RANGE_VIOLATION){
				fragment.addAll(analyzeRangeViolation(ax));
				currentState = State.CLASS_ASSERTION;
				if(stopIfInconsistencyFound && !fragment.isEmpty()){
					break;
				}
	...
	private Set<OWLAxiom> analyzeClassAssertionViolation(OWLDisjointClassesAxiom axiom){
	...
	private Set<OWLAxiom> analyzeDomainViolation(OWLDisjointClassesAxiom axiom){
		List<OWLClassExpression> classes = axiom.getClassExpressionsAsList();
		OWLClass cls1 = classes.get(0).asOWLClass();
		OWLClass cls2 = classes.get(1).asOWLClass();
		Set<OWLAxiom> axioms = new HashSet<>();
...
//			query = "SELECT ?p ?x ?y WHERE {?x a <" + cls1.toStringID() + ">. ?p rdfs:domain <" + cls2.toStringID() + ">. ?x ?p ?y}";
	...
	private Set<OWLAxiom> analyzeRangeViolation(OWLDisjointClassesAxiom axiom){
		List<OWLClassExpression> classes = axiom.getClassExpressionsAsList();
		OWLClass cls1 = classes.get(0).asOWLClass();
		OWLClass cls2 = classes.get(1).asOWLClass();
		Set<OWLAxiom> axioms = new HashSet<>();
