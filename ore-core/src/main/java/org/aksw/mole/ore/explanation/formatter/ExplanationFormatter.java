...
public class ExplanationFormatter {
	...
	private LoadingCache<Explanation, FormattedExplanation> cache = CacheBuilder.newBuilder()
		       .maximumSize(100)
		       .expireAfterWrite(10, TimeUnit.MINUTES)
		       .build(
		           new CacheLoader<Explanation, FormattedExplanation>() {
		             public FormattedExplanation load(Explanation explanation){
		               return format(explanation);
		             }
		           });
	
	public ExplanationFormatter(OWLOntologyManager manager) {
		orderer = new ExplanationOrdererImpl(manager);
	}
	
	public FormattedExplanation getFormattedExplanation(Explanation explanation){
		try {
	...
	private FormattedExplanation format(Explanation explanation){
	...
	public class FormattedExplanation {
		...
		public FormattedExplanation(Explanation explanation, List<OWLAxiom> orderedAxioms, Map<OWLAxiom, Integer> axiom2Indention) {
