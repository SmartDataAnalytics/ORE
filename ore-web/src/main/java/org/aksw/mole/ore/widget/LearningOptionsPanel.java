...
public class LearningOptionsPanel extends VerticalLayout{
	...
	private FlexibleOptionGroup profileOptions;
	...
	public LearningOptionsPanel() {
		...
		maxExecutionTimeSpinner.setWidth("80%");
		...
		maxNrOfResultsSpinner.setWidth("80%");
		...
		profileForm.setWidth("80%");
		profileForm.setCaption("OWL Profile");
		VerticalLayout vl = new VerticalLayout();
		vl.setSpacing(true);
		vl.setMargin(true);
		profileForm.setLayout(vl);
		
		profileOptions = new FlexibleOptionGroup(Arrays.asList(Profile.values()));
		profileOptions.setImmediate(true);
		HorizontalLayout optionGroupLayout = new HorizontalLayout();
		for (Iterator<FlexibleOptionGroupItemComponent> iter = profileOptions.getItemComponentIterator(); iter.hasNext();) {
		...
		profileOptions.addListener(new ValueChangeListener() {
			@Override
			public void valueChange(ValueChangeEvent event) {
				onProfileChanged();
			}
		});
		vl.addComponent(optionGroupLayout);
...
//		grid.setMargin(true);
		...
		grid.addComponent(universalQuantifierCheckBox, 0, 0);
		
		existentialQuantifierCheckBox = new CheckBox("some");
		grid.addComponent(existentialQuantifierCheckBox, 0, 1);
		
		negationCheckBox = new CheckBox("not");
		grid.addComponent(negationCheckBox, 0, 2);
		
		hasValueCheckBox = new CheckBox("value");
		grid.addComponent(hasValueCheckBox, 0, 3);
		...
		grid.addComponent(cardinalityLayout, 0, 4);
